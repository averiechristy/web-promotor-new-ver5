                        ...
                        <div class="form-group mb-4">
                            <div id="product-container">
                                ...
                                <div class="product-item">
                                    ...
                                    <select name="produk[]" class="produk-form product-select form-control {{$errors->has('produk.'.$index) ? 'is-invalid' : ''}}" style="border-color: #01004C;">
...
var oldProdukValue = {!! json_encode(old('produk')) !!}; // Ambil nilai old(produk)

// Fungsi untuk mengisi opsi produk berdasarkan roleId
function fillProductOptions(roleId) {
    $.ajax({
        ...
        success: function (data) {
            ...
            var oldProdukValueInteger = oldProdukValue.map(function (value) {
                return parseInt(value, 10); // Mengonversi elemen menjadi tipe data integer
            });

            $.each(data, function (key, produk) {
                var isSelected = oldProdukValueInteger.includes(produk.id); // Periksa apakah produk.id ada di dalam oldProdukValueInteger

                // Tambahkan kondisi untuk menentukan apakah opsi harus dipilih atau tidak
                var optionSelected = isSelected ? ' selected' : '';

                console.log("produk.id:", produk.id);
                console.log("oldProdukValue:", oldProdukValue);
                console.log("isSelected:", isSelected);

                productsSelect.append('<option value="' + produk.id + '"' + optionSelected + '>' + produk.kode_produk + " - " + produk.nama_produk + '</option>');
            });
        }
    });
}
    // Cek apakah $('#role') memiliki nilai yang sudah terpilih saat halaman dimuat
    var initialRoleId = $('#role').val();
    var initialProductId = $('.product-select').eq(0).val();
    if (initialRoleId) {
        // Jika ada nilai yang terpilih, panggil fungsi untuk mengisi opsi produk
        fillProductOptions(initialRoleId);
    }
    
    // Tambahkan event listener untuk merespons perubahan pada $('#role')
    $('#role').change(function () {
        var roleId = $(this).val();

        // Panggil fungsi untuk mengisi opsi produk saat terjadi perubahan
        fillProductOptions(roleId);
    });

   // Setelah mengisi ulang opsi produk berdasarkan role ID
        $('#role').change(function () {
            var roleId = $(this).val();
            $.ajax({
                url: '/getProduct/' + roleId,
        ...
        $('.add-product').click(function () {
            ...
            if (addedProducts[counter]) {
    productSelect.val(addedProducts[counter].id);
    quantityInput.val(addedProducts[counter].quantity);
        ...
        $(document).on('click', '.remove-product', function () {
            ...
            if (productContainer.children('.product-item').length > 1) {
                ...
                alert("Tidak dapat menghapus form produk pertama.");
            }
        });
